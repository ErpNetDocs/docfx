...
namespace Microsoft.DocAsCode.Build.ConceptualDocuments
{
    ...
    public class ProcessLists : BaseDocumentBuildStep
    {
        ...
        private static Dictionary<string, string> ParseYamlHeader(FileModel model)
        {
            ...
            var markdown = (string)content[Docfx.DataContracts.Common.Constants.PropertyName.Conceptual];
    ...
    internal static class PrivateFileModelExtensions
    {
        ...
        public static string? GetTitle(this FileModel model)
        {
            ...
            string markdown = (string)content[Docfx.DataContracts.Common.Constants.PropertyName.Conceptual];
            ...
            if (content.TryGetValue(Docfx.DataContracts.Common.Constants.PropertyName.TitleOverwriteH1, out metadataValue)
                && metadataValue is string titleOverwrite && !string.IsNullOrEmpty(titleOverwrite))
            {
                return titleOverwrite;
            }

            if (content.TryGetValue(Docfx.DataContracts.Common.Constants.PropertyName.Title, out metadataValue)
